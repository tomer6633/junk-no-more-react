...
function QuoteForm() {
  ...
  if (submitted) return (
    <div className="text-center py-12">
      ...
      <p className="text-gray-600">We'll call you back within 2 hours with a price and availability.</p>
    </div>
  )
  return (
    <form onSubmit={e => { e.preventDefault(); setSubmitted(true) }} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Type of Junk</label>
        <select required value={form.service} onChange={e => setForm({...form, service: e.target.value})}
          className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
          <option value="">What needs to go?</option>
      ...
      <div className="md:col-span-2">
        <label className="block text-sm font-medium text-gray-700 mb-1">Describe what needs to go</label>
        <textarea rows={3} placeholder="e.g. Old couch, broken fridge, boxes of stuff in the garage..." value={form.notes} onChange={e => setForm({...form, notes: e.target.value})}
    ...
    <div className="font-inter bg-white pb-16 md:pb-0">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <img src={business.heroImage} alt="Junk removal" className="w-full h-full object-cover" />
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-3xl mx-auto pt-20">
          <p className="font-inter text-brand-accent uppercase tracking-widest text-sm mb-4">London, Ontario Junk Removal</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={business.telLink} className="bg-brand-accent text-white font-oswald text-lg px-8 py-4 rounded-lg hover:opacity-90 transition">📞 Call for Same-Day Pickup</a>
      ...
      <section className="bg-brand-primary text-white py-10">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-3 gap-4 text-center">
          {[['Same-Day','Available'],['5★','Google Rating'],['No Hidden','Fees']].map(([v,l]) => (
      ...
      <section className="py-20 px-4 bg-brand-light" id="services">
        <div ref={s1} className="fade-in max-w-6xl mx-auto">
          <h2 className="font-oswald text-4xl text-brand-primary text-center mb-3">What We Remove</h2>
          <p className="text-center text-gray-500 mb-12">We haul almost anything — fast and for a fair price.</p>
      ...
      <section className="py-20 px-4 bg-white">
        <div ref={s2} className="fade-in max-w-5xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <img src={business.aboutImage} alt="Junk removal team" className="rounded-2xl shadow-xl w-full object-cover h-80" />
          <div>
            <p className="text-brand-accent font-inter uppercase tracking-widest text-sm mb-3">About Junk No More</p>
            <h2 className="font-oswald text-4xl text-brand-primary mb-5">London's Straightforward Junk Crew</h2>
            <p className="text-gray-600 leading-relaxed mb-4">No gimmicks, no surprise fees, no hassle. Call us, we show up, we take it away. That's it. London homeowners and businesses trust us because we actually do what we say.</p>
            <p className="text-gray-600 leading-relaxed mb-6">Fully insured. Locally owned. Items donated or recycled wherever possible.</p>
            <a href={business.telLink} className="inline-block bg-brand-primary text-white font-oswald px-6 py-3 rounded-lg hover:bg-brand-dark transition">📞 Call for Same-Day Service</a>
          </div>
        </div>
      </section>
      ...
      <section id="quote" className="py-20 px-4 bg-white">
        <div ref={s4} className="fade-in max-w-2xl mx-auto">
          ...
          <p className="text-center text-gray-500 mb-10">Tell us what needs to go and we'll get back to you within 2 hours.</p>
